...
from drift_fns import fast_n_sphere
...
def run_model_for_epochs(net, taskL, cfun, training, neps_per_task, n_context_steps, stim_v, verb=True):
  if training:
    net.train()
  else:
    net.eval()
...
def train_model_intervals(net,taskL,long_cfun,cfun,training,neps_per_task,
                          n_context_steps,model_name,stim_v,save_intermediate=False, verb=True):
  if training:
    net.train()
  else:
    net.eval()
